...
parser.add_argument("--load_pretrain_par_file", default='.\\save_result\\A5_best_model_08.pth',
                    type=str, help="file to save the pretrain best model")

parser.add_argument("--save_file", default='.\\save_result\\FreeSolv_A5_08.pth',
                    type=str, help="file to save the best downstream model")

parser.add_argument("--save_seedR2", default='.\\save_result\\seed_R2_A5_08.csv',
                    type=str, help="file to save the best downstream model")
# ————————————————————————————————————————————————————————————————————————————————————————————————
...
random_seed = np.random.randint(20,2000,30)
result = np.zeros_like(random_seed, dtype=float)
for aa, ss in enumerate(random_seed):
    seed_set(ss)
...
rr = zip(random_seed, result)
dframe = pd.DataFrame(rr, columns=['random_seed', 'R2'])
dframe.to_csv(args.save_seedR2, index=False)