...
class BERT(nn.Module):
    ...
    def __init__(self, atom_vocab_size = 17, nmr_vocab_size = 1000, embed_dim = 128, ffn_dim = 256, head = 8, encoder_layers = 6, dropout=0.1):
        ...
        self.transformer_blocks = nn.ModuleList([TransformerBlock(head=head, embed_dim=embed_dim, d_ff=embed_dim*4, dropout=dropout) for _ in range(encoder_layers)])

        self.ff_nmr = nn.ModuleList([nn.Linear(embed_dim, ffn_dim), nn.ReLU(), nn.Dropout(p=0),
                                     nn.Linear(ffn_dim, 1)])
