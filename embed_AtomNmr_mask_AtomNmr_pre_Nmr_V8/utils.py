...
class Tokenizer():
    ...
    def convert_atom_to_id(self, atom: str) -> int:  # 单个字符转换
        return self.vocab.get(atom, self.vocab['<unk>'])  # 若atom不存在于字典vocab中，则用<unk>代替

    def convert_atoms_to_ids(self, mol: List[str]) -> List[int]: # list中字符转换
...
def Smiles_to_adjoin(smiles, explicit_hydrogens=True, canonical_atom_order=False):
    ...
    num_atoms = mol.GetNumAtoms() #一个分子中所有原子的个数
